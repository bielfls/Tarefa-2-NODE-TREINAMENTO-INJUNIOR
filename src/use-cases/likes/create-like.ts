...
export class CreateLikeUseCase {
  ...
  async execute({ userId, postId, commentId }: CreateLikeUseCaseRequest) {
    if (!postId && !commentId) {
      throw new Error(
        'É necessário informar o ID de um post ou de um comentário.',
      )
    }

    // biome-ignore lint/suspicious/noImplicitAnyLet: a variável like pode ser do tipo Like ou null, dependendo do contexto
    let like
