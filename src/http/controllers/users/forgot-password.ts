...
export async function forgotPassword(
  ...
  try {
    ...
    return reply.status(200).send({
      message:
        'Se o e-mail tiver login, um link de recuperação será enviado em instantes.',
    })
  } catch (error) {
    ...
    if (error instanceof ResourceNotFoundError) {
      return reply.status(200).send({
        message:
          'Se o e-mail estiver cadastrado, um link de recuperação será enviado em instantes.',
      })
    }
